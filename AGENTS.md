...
- **Der Status steht in der Datei, nicht in einer Regel über die Datei.** Ein Fehlen ist beim Lesen unsichtbar: Wer eine Datei öffnet, sieht ihr Anlagedatum nicht und weiss deshalb nicht, ob das fehlende `geprueft:` "Vorschlag" bedeutet oder nur "alt". Genau so wird aus einem unbestätigten Ergebnis über zwei, drei Sessions zitierte Beschlusslage. Deshalb trägt jede Wissensdatei in einem Themenordner ihren Status sichtbar in sich: entweder `geprueft: <Kürzel> <Datum>` im Frontmatter, oder einen Satz im Text, der sagt, dass sie unbestätigt ist ("Von <Agent> aus den genannten Quellen erarbeitet, von `[NAME]` nicht bestätigt"). Der Satz behauptet nichts, er macht sichtbar, was ohnehin gilt. Mechanisch geprüft im System-Health-Check, Kategorie `Herkunft`; Rohquellen sind ausgenommen, denn eine unveränderte Quelle ist der Beleg selbst. Die beiden anderen Herkunftsregeln dieses Abschnitts (Quelle zitieren, eigene Schlüsse im Satz markieren) bleiben eine Sorgfaltspflicht des schreibenden Modells, weil kein Skript sie sehen kann.
